...
class ArcballControl {
    ...
    constructor(canvas, updateCallback) {
        ...
        canvas.addEventListener('touchstart', e => {
            ...
            vec2.set(this.pointerPos, touch.clientX, touch.clientY);
            vec2.copy(this.previousPointerPos, this.pointerPos);
            this.isPointerDown = true;
            startTouch = touch;
        });
    ...
    #initTexture() {
        ...
        Promise.all(
            this.items.map(item => new Promise(resolve => {
                ...
                const resolveImg = () => resolve(img);
                img.onload = resolveImg;
                img.onerror = resolveImg; // Resolve even on error to prevent blocking
                img.src = item.image;
            }))
        ).then(images => {
            images.forEach((img, i) => {
                ...
                if (img.width > 0) ctx.drawImage(img, x, y, cellSize, cellSize);
            });
...
const defaultReviews = [
    {
        image: 'https://images.guns.lol/e7d2c74909113866fff54169156ad21fd9732a78/HzJVMD.jpg',
        title: 'Customer Comment',
        description: 'The Ardent Loading Screen could be the best $10 i have spent so far.',
        author: 'Cat.',
        link: 'https://www.ardentdev.shop/products/ardent-loading-screen'
    },
    {
        image: 'https://images.guns.lol/e7d2c74909113866fff54169156ad21fd9732a78/b5YtOz.jpg',
        title: 'Customer Comment',
        description: 'i think its good.',
        author: 'rebeca',
        link: 'https://www.ardentdev.shop/products/ardent-loading-screen'
    },
    {
        image: 'https://images.guns.lol/e7d2c74909113866fff54169156ad21fd9732a78/RSU3ra.jpg',
        title: 'Customer Comment',
        description: 'My old Loading Screen was so bad, Im so glad i found this!!!',
        author: 'Freaky Cat.',
        link: 'https://www.ardentdev.shop/products/ardent-loading-screen'
    },
    {
        image: 'https://images.guns.lol/e7d2c74909113866fff54169156ad21fd9732a78/H7Rg8H.jpg',
        title: 'Customer Comment',
        description: 'I control all the lightning!!!',
        author: 'Strong Cat.',
        link: 'https://www.ardentdev.shop/products/ardent-loading-screen'
    },
    {
        image: 'https://images.guns.lol/e7d2c74909113866fff54169156ad21fd9732a78/8Pq8O8.jpg',
        title: 'Customer Comment',
        description: 'I love the design, it is so clean and simple.',
        author: 'Person',
        link: 'https://www.ardentdev.shop/products/ardent-loading-screen'
    }
];

window.reviewSketchInitialized = false;
window.reviewSketch = null;

function initReviews() {
    const canvas = document.getElementById('infinite-grid-menu-canvas');
    if (!canvas) return;

    const cardEl = document.getElementById('review-horizontal-card');
    ...
    const btnEl = document.getElementById('review-btn');

    if (cardEl) cardEl.className = 'review-horizontal-card inactive';

    let activeReview = null;

    const onActiveItemChange = (index) => {
        const itemIndex = index % defaultReviews.length;
        activeReview = defaultReviews[itemIndex];

        if (activeReview) {
            titleEl.textContent = activeReview.title;
            descEl.textContent = `"${activeReview.description}"`;
            if (authorEl) authorEl.textContent = activeReview.author;
            if (avatarEl) avatarEl.src = activeReview.image;

            if (activeReview.link && activeReview.link.startsWith('http')) {
                btnEl.href = activeReview.link;
            } else {
                btnEl.href = '#';
            }
        }
    ...
    const onMovementChange = (isMoving) => {
        if (!cardEl) return;
        if (isMoving || !activeReview) {
            cardEl.className = 'review-horizontal-card inactive';
        } else {
            cardEl.className = 'review-horizontal-card active';
        }
    };

    window.reviewSketch = new InfiniteGridMenu(
        canvas,
        defaultReviews,
        onActiveItemChange,
        onMovementChange,
    ...
    // Force showing the first element initially
    onActiveItemChange(0);
    onMovementChange(false);

    window.addEventListener('resize', () => {
        if (window.reviewSketch) window.reviewSketch.resize();
    });
}
